...
export const SECURITY_RULES: RuleDefinition[] = [
  ...
  // 12. Weak Password PBKDF2 Iterations
  {
    ...
    pattern: /(?:pbkdf2|hash_password|derive_key|bcrypt)\s*\([^)]*(?:iterations\s*=\s*(?:[1-9][0-9]{0,3}|[1-5][0-9]{4})\b|,\s*(?:[1-9][0-9]{0,3}|[1-5][0-9]{4})\s*,)/i,
  ...
  // 18. Cleartext Storage of Sensitive Data
  {
    ...
    pattern: /(?:localStorage\.setItem\s*\(\s*["'](?:token|password|auth|credit_card|ssn)["']|http:\/\/(?!localhost|127\.0\.0\.1))/i,
